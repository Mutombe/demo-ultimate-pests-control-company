...
function HeroSection() {
  ...
  return (
    <section ref={containerRef} className="relative h-screen min-h-[700px] overflow-hidden bg-gray-950">
      <motion.div className="absolute inset-0" style={{ y: bgY }}>
        ...
        <div className="absolute inset-0 bg-gradient-to-b from-gray-950/85 via-gray-950/50 to-gray-950/90 z-[1]" />
        <div className="absolute inset-0 bg-gradient-to-r from-gray-950/80 via-transparent to-gray-950/30 z-[1]" />
      </motion.div>
      <NoiseTexture opacity={0.04} />

      {/* Red warning accent */}
      <div className="absolute top-[15%] left-0 w-[3px] h-32 sm:h-48 bg-gradient-to-b from-transparent via-red-500 to-transparent z-20" />

      {/* Guarantee badge */}
      <div className="absolute top-28 right-6 sm:right-10 z-30 bg-red-600 text-white p-4 shadow-xl">
        <div className="text-center">
          <ShieldCheck size={24} weight="fill" className="mx-auto mb-1" />
          <div className="text-[9px] font-bold uppercase tracking-[0.15em]">Zero Pest</div>
          <div className="text-[8px] uppercase tracking-wider text-red-200">Guarantee</div>
        </div>
      </div>
      ...
      <motion.div className="relative z-20 flex flex-col justify-center h-full max-w-7xl mx-auto px-5 sm:px-8 lg:px-12 pt-36" style={{ y: textY, opacity }}>
        <motion.div initial={{ scaleX: 0 }} animate={{ scaleX: 1 }} transition={{ duration: 1, delay: 0.3 }}
          className="w-16 h-[2px] bg-red-500 mb-6 origin-left" />
        ...
        <div className="overflow-hidden">
          {['ZERO', 'TOLERANCE', 'FOR PESTS.'].map((line, i) => (
            <motion.div key={line} initial={{ y: '110%' }} animate={{ y: 0 }} transition={{ duration: 1, delay: 0.5 + i * 0.15, ease: [0.16, 1, 0.3, 1] }}>
              <h1 className={`font-heading leading-[0.92] tracking-tight ${line === 'TOLERANCE' ? 'text-red-500 italic' : 'text-white'}`}
                style={{ fontSize: 'clamp(2.2rem, 7vw, 4.5rem)', fontWeight: line === 'TOLERANCE' ? 700 : 300 }}>{line}</h1>
            </motion.div>
          ))}
        </div>
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.2 }} className="flex items-center gap-3 mt-6">
          <div className="flex items-center gap-1.5 bg-red-500/10 border border-red-500/30 px-3 py-1.5">
            <Clock size={12} className="text-red-400" />
            <span className="text-red-300 text-[10px] font-bold uppercase tracking-wider">48hr Response Time</span>
          </div>
        </motion.div>
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.4 }} className="flex flex-wrap gap-4 mt-10">
          <Link to="/contact" className="group inline-flex items-center gap-3 bg-red-600 text-white px-8 py-4 text-sm uppercase tracking-[0.15em] font-semibold hover:bg-red-700 hover:shadow-xl hover:shadow-red-600/20 transition-all" style={{ fontFamily: 'var(--font-sans)' }}>
            {hero.ctaPrimary} <ArrowRight size={16} className="group-hover:translate-x-1 transition-transform" />
          </Link>
          <a href={`tel:${business.phoneRaw}`} className="group inline-flex items-center gap-3 border border-white/15 text-white px-8 py-4 text-sm uppercase tracking-[0.15em] font-semibold hover:border-red-500/50 hover:text-red-400 transition-all" style={{ fontFamily: 'var(--font-sans)' }}>
            <Phone size={18} weight="fill" /> Emergency Call
          </a>
        </motion.div>
      </motion.div>
...
function GuaranteeBadge() {
  ...
  const inView = useInView(ref, { once: true, margin: '-50px' });
  return (
    <section ref={ref} className="bg-red-600 py-8">
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12">
        <motion.div initial={{ opacity: 0 }} animate={inView ? { opacity: 1 } : {}} transition={{ duration: 0.6 }}
          ...
          <div>
            <div className="text-white text-sm font-bold uppercase tracking-[0.15em]" style={{ fontFamily: 'var(--font-sans)' }}>100% Satisfaction Guarantee</div>
            <div className="text-white/70 text-xs mt-1" style={{ fontFamily: 'var(--font-sans)' }}>If pests return within 30 days, we re-treat at no extra cost.</div>
          </div>
        </motion.div>
      </div>
    </section>
...
function MarqueeTicker() {
  ...
  return (
    <section className="bg-gray-950 border-y border-red-500/10 py-5 sm:py-6 overflow-hidden">
      <div className="animate-marquee flex whitespace-nowrap">
        {repeated.map((item, i) => (
          <span key={i} className="flex items-center gap-6 sm:gap-8 mx-6 sm:mx-8">
            <span className="text-red-400/80 font-heading text-lg sm:text-2xl italic tracking-wider">{item}</span>
            <span className="text-red-500/20 text-sm">&diams;</span>
          </span>
        ))}
      </div>
    </section>
...
function PestTypes() {
  ...
  const pests = ['Cockroaches', 'Termites', 'Rodents', 'Bed Bugs', 'Mosquitoes', 'Ants'];
  return (
    <section ref={ref} className="bg-gray-900 py-20 sm:py-28">
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.8 }} className="text-center mb-14">
          ...
          <h2 className="font-heading text-white leading-[0.95] italic" style={{ fontSize: 'clamp(2rem, 4.5vw, 3rem)' }}>
            Pests We <span className="text-red-500">Eliminate</span>
          </h2>
        </motion.div>
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
          {pests.map((pest, i) => (
            <motion.div key={pest} initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}}
              transition={{ delay: i * 0.08 }} className="p-6 border border-white/10 text-center hover:border-red-500/50 transition-colors group">
              <div className="w-12 h-12 bg-red-500/10 flex items-center justify-center mx-auto mb-4 group-hover:bg-red-500/20 transition-colors">
                <CheckCircle size={20} weight="fill" className="text-red-500" />
              </div>
              <h3 className="text-white font-heading text-base italic">{pest}</h3>
            </motion.div>
          ))}
        </div>
      </div>
...
function ServicesGrid() {
  ...
  return (
    <section ref={ref} className="bg-gray-900 py-24 sm:py-32 lg:py-40">
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.8 }} className="mb-14 sm:mb-20">
          ...
          <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-6">
            <div>
              <p className="text-red-400/60 text-xs uppercase tracking-[0.3em] mb-3" style={{ fontFamily: 'var(--font-sans)' }}>Our Services</p>
              ...
              <motion.div key={service.title} initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}}
                ...
                <Link to={`/services#${services?.items?.[i]?.slug || ''}`}
                  ...
                  <div className="absolute top-4 right-5 z-10">
                    <span className="text-red-500/15 font-heading text-6xl sm:text-7xl italic leading-none">{String(i + 1).padStart(2, '0')}</span>
                  </div>
                  <div className="absolute top-5 left-5 z-10 w-10 h-10 border border-red-500/30 flex items-center justify-center bg-gray-950/60 backdrop-blur-sm group-hover:bg-red-500/20 group-hover:border-red-500/60 transition-all duration-500">
                    <IconComp size={18} weight="fill" className="text-red-500" />
                  </div>
                  ...
                  <div className="absolute bottom-0 left-0 right-0 h-[2px] bg-gradient-to-r from-red-500 to-red-300 z-10" />
                </Link>
              </motion.div>
            );
          }}
...
function PortfolioGallery() {
  ...
  return (
    <section ref={containerRef} className="bg-gray-950 py-24 sm:py-32 lg:py-40 overflow-hidden">
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12 mb-12 sm:mb-16">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}}
          ...
          <div>
            <div className="w-12 h-[2px] bg-red-500 mb-6" />
            <p className="text-red-400/60 text-xs uppercase tracking-[0.3em] mb-3" style={{ fontFamily: 'var(--font-sans)' }}>Selected Works</p>
          ...
          {projects.items.map((project, i) => (
            <div key={project.slug} className="group relative flex-shrink-0 w-[300px] sm:w-[360px] lg:w-[420px] overflow-hidden">
              <div className="aspect-[3/4] overflow-hidden relative">
                ...
                <div className="absolute inset-0 border-2 border-transparent group-hover:border-red-500/40 transition-colors duration-500 z-10" />
                <div className="absolute top-5 left-5 z-10">
                  <span className="bg-red-500/90 text-gray-950 text-[10px] uppercase tracking-[0.2em] px-3 py-1.5 font-semibold" style={{ fontFamily: 'var(--font-sans)' }}>
                    {project.category}
                  </span>
                </div>
              </div>
    ...
    <section ref={ref} className="bg-gray-900 py-24 sm:py-32">
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}}
          ...
          <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-6">
            <div>
              <p className="text-red-400/60 text-xs uppercase tracking-[0.3em] mb-3" style={{ fontFamily: 'var(--font-sans)' }}>Recent Work</p>
              <h2 className="font-heading text-white leading-[0.92] italic" style={{ fontSize: 'clamp(2rem, 4.5vw, 3.5rem)' }}>
          ...
          {projects.items.slice(0, 6).map((project, i) => (
            <motion.div key={project.slug} initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}}
              ...
              <div className={`overflow-hidden relative ${i === 0 ? 'aspect-square sm:aspect-auto sm:h-full' : 'aspect-[4/3]'}`}>
                ...
                <div className="absolute inset-0 border-2 border-transparent group-hover:border-red-500/30 transition-colors duration-500 z-10" />
                ...
                {/* Number watermark */}
                <div className="absolute top-4 right-5 z-10">
                  <span className="text-red-500/10 font-heading text-5xl italic leading-none">{String(i + 1).padStart(2, '0')}</span>
                </div>
                ...
                {/* Bottom accent */}
                <div className="absolute bottom-0 left-0 right-0 h-[2px] bg-red-500 z-10 scale-x-0 group-hover:scale-x-100 transition-transform duration-500 origin-left" />
              </div>
            </motion.div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-10 sm:gap-16">
          {stats.map((stat, i) => (
            <motion.div key={stat.label} initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}}
              ...
              <div className="font-heading text-red-500 leading-none italic" style={{ fontSize: 'clamp(2.8rem, 6vw, 5rem)' }}>
              ...
              {i < stats.length - 1 && (
                <div className="hidden lg:block absolute right-0 top-1/2 -translate-y-1/2 h-16 w-[1px] bg-gradient-to-b from-transparent via-red-500/15 to-transparent" />
              )}
            </motion.div>
          ))}
        </div>
...
function AboutSection() {
  ...
  return (
    <section ref={ref} className="bg-gray-950 py-24 sm:py-32 lg:py-40 overflow-hidden">
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12">
        <div className="grid lg:grid-cols-2 gap-16 lg:gap-24 items-center">
          <motion.div initial={{ opacity: 0, x: -40 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.9 }}>
            <div className="w-12 h-[2px] bg-red-500 mb-6" />
            <p className="text-red-400/60 text-xs uppercase tracking-[0.3em] mb-3" style={{ fontFamily: 'var(--font-sans)' }}>Our Story</p>
            <h2 className="font-heading text-white leading-[0.95] italic mb-8" style={{ fontSize: 'clamp(2rem, 5vw, 3.5rem)' }}>
              The Ultimate<br /><span className="text-red-500">Story</span>
            </h2>
            {about.story.map((p, i) => (
            ...
            <div className="flex gap-10 sm:gap-16">
              <div>
                <div className="text-red-500 font-heading text-3xl sm:text-4xl italic leading-none">{business.established}</div>
                <div className="text-white/30 text-[10px] uppercase tracking-[0.2em] mt-2" style={{ fontFamily: 'var(--font-sans)' }}>Founded</div>
              </div>
              <div>
                <div className="text-red-500 font-heading text-3xl sm:text-4xl italic leading-none">{business.projectsCompleted}</div>
                <div className="text-white/30 text-[10px] uppercase tracking-[0.2em] mt-2" style={{ fontFamily: 'var(--font-sans)' }}>Completed</div>
              </div>
              <div>
                <div className="text-red-500 font-heading text-3xl sm:text-4xl italic leading-none">{business.yearsExperience}</div>
          ...
          <motion.div initial={{ opacity: 0, x: 40 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.9, delay: 0.2 }} className="relative">
            ...
            <div className="absolute -bottom-3 -right-3 w-16 h-16 border-b-2 border-r-2 border-red-500/30" />
          </motion.div>
        </div>
      </div>
    </section>
...
function WhyChooseUs() {
  ...
  return (
    <section ref={ref} className="bg-gray-900 py-24 sm:py-32 lg:py-40 overflow-hidden">
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12">
        <div className="grid lg:grid-cols-2 gap-16 lg:gap-20 items-center">
          ...
          <motion.div initial={{ opacity: 0, x: 40 }} animate={inView ? { opacity: 1, x: 0 } : {}}
            ...
            <div className="w-12 h-[2px] bg-red-500 mb-6" />
            <p className="text-red-400/60 text-xs uppercase tracking-[0.3em] mb-3" style={{ fontFamily: 'var(--font-sans)' }}>The Difference</p>
            <h2 className="font-heading text-white leading-[0.95] italic mb-12" style={{ fontSize: 'clamp(2rem, 4.5vw, 3rem)' }}>
              Why Choose <span className="text-red-500">Ultimate</span>
            </h2>
              ...
              {points.map((point, i) => (
                <motion.div key={point.title} initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}}
                  ...
                  <div className="shrink-0 mt-1">
                    <div className="w-8 h-8 border border-red-500/30 flex items-center justify-center bg-red-500/5">
                      <CheckCircle size={16} weight="fill" className="text-red-500" />
                    </div>
                  </div>
                  <div>
...
function ValuesGrid() {
  ...
  return (
    <section ref={ref} className="bg-gray-950 py-24 sm:py-32">
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}}
          transition={{ duration: 0.8 }} className="text-center mb-14 sm:mb-20">
          <div className="w-12 h-[2px] bg-red-500 mx-auto mb-6" />
          <p className="text-red-400/60 text-xs uppercase tracking-[0.3em] mb-3" style={{ fontFamily: 'var(--font-sans)' }}>What Drives Us</p>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {about.values.map((value, i) => (
            <motion.div key={value.title} initial={{ opacity: 0, y: 25 }} animate={inView ? { opacity: 1, y: 0 } : {}}
              ...
              className="relative p-6 sm:p-8 border border-white/10 hover:border-red-500/40 transition-all duration-500 group">

              {/* Number watermark */}
              <span className="absolute top-4 right-4 text-red-500/8 font-heading text-6xl italic leading-none">
                {String(i + 1).padStart(2, '0')}
              </span>

              {/* Icon badge */}
              <div className="w-12 h-12 bg-red-500/10 flex items-center justify-center mb-5 group-hover:bg-red-500/20 transition-colors duration-500">
                <CheckCircle size={22} weight="fill" className="text-red-500" />
              </div>
...
function ResponseCounter() {
  ...
  const inView = useInView(ref, { once: true, margin: '-50px' });
  return (
    <section ref={ref} className="bg-gray-950 py-12">
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12">
        <motion.div initial={{ opacity: 0 }} animate={inView ? { opacity: 1 } : {}} transition={{ duration: 0.6 }}
          className="flex flex-col sm:flex-row items-center justify-center gap-8">
          {[{ val: '48hr', lbl: 'Response Time' }, { val: '100%', lbl: 'Satisfaction' }, { val: '30-Day', lbl: 'Guarantee' }].map((item, i) => (
            <div key={i} className="text-center">
              <div className="text-red-500 font-heading text-3xl sm:text-4xl italic leading-none">{item.val}</div>
              <div className="text-white/30 text-[10px] uppercase tracking-[0.2em] mt-2" style={{ fontFamily: 'var(--font-sans)' }}>{item.lbl}</div>
            </div>
          ))}
        </motion.div>
      </div>
...
function TestimonialsSection() {
  ...
  return (
    <section ref={ref} className="relative bg-gray-950 py-24 sm:py-32 lg:py-40 overflow-hidden">
            ...
            <motion.div key={active} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.6 }}>
              ...
              <div className="flex flex-col items-center gap-3">
                ...
                <div className="flex items-center gap-0.5 mt-1">
                  {[...Array(t.rating)].map((_, i) => <Star key={i} size={12} weight="fill" className="text-red-500" />)}
                </div>
              </div>
            </motion.div>
          </AnimatePresence>
          <div className="flex items-center justify-center gap-6 mt-12">
            <button onClick={prev} className="w-10 h-10 border border-white/10 flex items-center justify-center text-white/30 hover:text-red-500 hover:border-red-500/30 transition-colors" aria-label="Previous">
              <CaretLeft size={16} />
            </button>
            <div className="flex gap-2">
              {homeTestimonials.map((_, i) => (
                <button key={i} onClick={() => setActive(i)} className={`h-[2px] transition-all duration-500 ${i === active ? 'w-10 bg-red-500' : 'w-3 bg-white/10'}`} aria-label={`Testimonial ${i + 1}`} />
              ))}
            </div>
            <button onClick={next} className="w-10 h-10 border border-white/10 flex items-center justify-center text-white/30 hover:text-red-500 hover:border-red-500/30 transition-colors" aria-label="Next">
              <CaretRight size={16} />
            </button>
          </div>
        </motion.div>
...
function CTASection() {
  ...
  return (
    <section ref={ref} className="relative py-28 sm:py-36 lg:py-48 overflow-hidden">
      <motion.div className="absolute inset-0" style={{ y: bgY }}>
        <img src={siteData.hero.backgroundImages[0]?.url} alt="Ultimate pests control company" className="w-full h-[130%] object-cover object-center" loading="lazy" />
      ...
      <div className="relative z-20 max-w-5xl mx-auto px-5 sm:px-8 lg:px-12 text-center">
        <motion.div initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: '-80px' }} transition={{ duration: 1 }}>
          <div className="w-16 h-[2px] bg-red-500 mx-auto mb-8" />
          <h2 className="font-heading text-white leading-[0.92] italic mb-8" style={{ fontSize: 'clamp(2.5rem, 8vw, 6rem)' }}>
            READY TO<br /><span className="text-red-500">GET STARTED?</span>
          </h2>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <Link to="/contact" className="group relative inline-flex items-center gap-3 bg-red-500 text-gray-950 px-8 py-4 text-sm uppercase tracking-[0.15em] font-semibold hover:shadow-xl transition-all" style={{ fontFamily: 'var(--font-sans)' }}>
          ...
          <div className="mt-16 pt-8 border-t border-white/5 flex flex-col sm:flex-row items-center justify-center gap-6 text-xs text-white/25 uppercase tracking-[0.15em]" style={{ fontFamily: 'var(--font-sans)' }}>
            <a href={`tel:${business.phoneRaw}`} className="flex items-center gap-2 hover:text-red-500 transition-colors"><Phone size={14} /> {business.phone}</a>
