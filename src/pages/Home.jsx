  ...
  return (
    <section ref={containerRef} className="relative h-screen min-h-[700px] overflow-hidden bg-slate-950">
      ...
      <div className="absolute right-5 sm:right-8 top-1/2 -translate-y-1/2 z-30 flex flex-col gap-3">
        {heroImages.map((_, i) => (
          <button key={i} onClick={() => setCurrentSlide(i)} className={`w-[3px] transition-all duration-700 ${i === currentSlide ? 'h-10 bg-emerald-500' : 'h-4 bg-white/20 hover:bg-white/40'}`} aria-label={`Slide ${i + 1}`} />
        ))}
      </div>
      ...
      {/* Left accent line */}
      <div className="absolute top-[15%] left-0 w-[2px] h-32 sm:h-48 bg-gradient-to-b from-transparent via-emerald-500 to-transparent z-20" />
      ...
      <motion.div className="relative z-20 flex flex-col justify-center h-full max-w-7xl mx-auto px-5 sm:px-8 lg:px-12 pt-36" style={{ y: textY, opacity }}>
        <motion.div initial={{ scaleX: 0 }} animate={{ scaleX: 1 }} transition={{ duration: 1, delay: 0.3, ease: 'easeOut' }} className="w-16 h-[2px] bg-gradient-to-r from-emerald-500 to-emerald-400/50 mb-6 origin-left" />

        <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.4 }} className="text-emerald-400 text-xs sm:text-sm font-semibold uppercase tracking-[0.3em] mb-8" style={{ fontFamily: 'var(--font-sans)' }}>
          {hero.badge}
        </motion.p>

        <div className="overflow-hidden">
          {['THE ULTIMATE', 'DEFENCE', 'AGAINST PESTS.'].map((line, i) => (
            <motion.div key={line} initial={{ y: '110%' }} animate={{ y: 0 }} transition={{ duration: 1, delay: 0.5 + i * 0.15, ease: [0.16, 1, 0.3, 1] }}>
              <h1
                className={`font-heading leading-[0.92] tracking-tight ${line === 'DEFENCE' ? 'bg-gradient-to-r from-emerald-500 via-emerald-400 to-emerald-500 bg-clip-text text-transparent italic' : 'text-white'}`}
                style={{ fontSize: 'clamp(2.2rem, 7vw, 4.5rem)', fontWeight: line === 'DEFENCE' ? 700 : 300 }}
              >
                {line}
              </h1>
            </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 1.4 }} className="flex flex-wrap gap-4 mt-10">
          <Link to="/contact" className="group relative inline-flex items-center gap-3 bg-emerald-500 text-white px-8 py-4 text-sm uppercase tracking-[0.15em] font-semibold transition-all duration-500 hover:bg-emerald-400 hover:shadow-xl hover:shadow-emerald-500/20" style={{ fontFamily: 'var(--font-sans)' }}>
            {hero.ctaPrimary}
            <ArrowRight size={16} className="transition-transform group-hover:translate-x-1" />
          </Link>
          <Link to="/services" className="group inline-flex items-center gap-3 border border-white/20 text-white px-8 py-4 text-sm uppercase tracking-[0.15em] font-semibold transition-all duration-500 hover:border-emerald-500/50 hover:text-emerald-400 hover:bg-white/5" style={{ fontFamily: 'var(--font-sans)' }}>
            {hero.ctaSecondary}
          </Link>
        </motion.div>
      </motion.div>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 2.5 }} className="absolute bottom-8 sm:bottom-12 left-1/2 -translate-x-1/2 z-20 flex flex-col items-center gap-2">
        ...
        <motion.div animate={{ y: [0, 8, 0] }} transition={{ repeat: Infinity, duration: 2, ease: 'easeInOut' }}>
          <ArrowDown size={14} className="text-emerald-500/40" />
        </motion.div>
      </motion.div>
      ...
      <div className="hidden lg:flex absolute right-8 bottom-12 z-20">
        <span className="text-white/10 text-[10px] uppercase tracking-[0.4em]" style={{ writingMode: 'vertical-rl', fontFamily: 'var(--font-sans)' }}>
          Ultimate pests control company &mdash; Paisley Rd
        </span>
      </div>
    </section>
  );
...
function MarqueeTicker() {
  const items = ['DEEP CLEANING', 'PEST CONTROL', 'FUMIGATION', 'OFFICE CLEANING', 'SANITISATION', 'CARPET CARE'];
  const repeated = [...items, ...items, ...items, ...items];

  return (
    <section className="bg-slate-950 border-y border-emerald-500/10 py-5 sm:py-6 overflow-hidden">
      <div className="animate-marquee flex whitespace-nowrap">
        {repeated.map((item, i) => (
          <span key={i} className="flex items-center gap-6 sm:gap-8 mx-6 sm:mx-8">
            <span className="text-emerald-500/80 font-heading text-lg sm:text-2xl italic tracking-wider">{item}</span>
            <span className="text-emerald-500/20 text-sm">&diams;</span>
          </span>
        ))}
      </div>
    </section>
      ...
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.8 }} className="mb-14 sm:mb-20">
          ...
          <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-6">
            <div>
              <p className="text-emerald-500/60 text-xs uppercase tracking-[0.3em] mb-3" style={{ fontFamily: 'var(--font-sans)' }}>Our Services</p>
              <h2 className="font-heading text-white leading-[0.92] italic" style={{ fontSize: 'clamp(2rem, 4.5vw, 3.5rem)' }}>
                What We <span className="text-emerald-500">Deliver</span>
              </h2>
            </div>
            <Link to="/services" className="group text-white/30 text-xs uppercase tracking-[0.2em] flex items-center gap-2 hover:text-emerald-500 transition-colors" style={{ fontFamily: 'var(--font-sans)' }}>
            ...
            return (
              <motion.div key={service.title} initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.7, delay: 0.08 * i }} className={i === 0 ? 'sm:col-span-2 lg:col-span-2' : ''}>
                <Link to={`/services#${services?.items?.[i]?.slug || ''}`} className={`group relative block overflow-hidden ${i === 0 ? 'aspect-[16/9] sm:aspect-[2/1]' : 'aspect-[3/4]'}`}>
                  ...
                  {/* Icon badge */}
                  <div className="absolute top-5 left-5 z-10 w-10 h-10 border border-emerald-500/30 flex items-center justify-center bg-slate-950/60 backdrop-blur-sm group-hover:bg-emerald-500/20 group-hover:border-emerald-500/60 transition-all duration-500">
                    <IconComp size={18} weight="light" className="text-emerald-500" />
                  </div>
                  ...
                  <div className="absolute bottom-0 left-0 right-0 h-[2px] bg-gradient-to-r from-emerald-500 to-emerald-400 z-10" />
                </Link>
              </motion.div>
            );
          })}
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-10 sm:gap-16">
          {stats.map((stat, i) => (
            <motion.div key={stat.label} initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.7, delay: i * 0.12 }} className="text-center relative">
              <div className="font-heading text-emerald-500 leading-none italic" style={{ fontSize: 'clamp(2.8rem, 6vw, 5rem)', textShadow: '0 0 40px rgba(34,197,94,0.15)' }}>
      ...
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12">
        <div className="grid lg:grid-cols-2 gap-16 lg:gap-24 items-center">
          <motion.div initial={{ opacity: 0, x: -40 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.9, ease: 'easeOut' }}>
            <div className="w-12 h-[2px] bg-emerald-500 mb-6" />
            <p className="text-emerald-500/60 text-xs uppercase tracking-[0.3em] mb-3" style={{ fontFamily: 'var(--font-sans)' }}>Our Story</p>
            <h2 className="font-heading text-white leading-[0.95] italic mb-8" style={{ fontSize: 'clamp(2rem, 5vw, 3.5rem)' }}>
              About<br /><span className="text-emerald-500">{business.name}</span>
            </h2>
            ...
            <div className="flex gap-10 sm:gap-16">
              ...
              <div>
                <div className="text-emerald-500 font-heading text-3xl sm:text-4xl italic leading-none">{business.employees}</div>
          ...
          <motion.div initial={{ opacity: 0, x: 40 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.9, delay: 0.2, ease: 'easeOut' }} className="relative">
            <div className="relative">
              ...
              <div className="absolute -top-4 -right-4 sm:-right-6 bg-emerald-500 text-white p-5 sm:p-7 shadow-2xl">
...
function WhyChooseUs() {
  ...
  const points = [
    { title: 'Certified Technicians', desc: 'Every team member is trained and certified in professional cleaning and pest control methods.' },
    { title: 'Eco-Friendly Products', desc: 'We use environmentally safe products that are tough on dirt and pests but gentle on your family.' },
    { title: '100% Satisfaction Guarantee', desc: 'Not happy? We come back and redo the job at no extra cost. That is our promise.' },
    { title: 'Flexible Scheduling', desc: 'We work around your timetable -- early mornings, evenings, weekends. You choose, we show up.' },
  ];
      ...
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12">
        <div className="grid lg:grid-cols-2 gap-16 lg:gap-20 items-center">
          ...
          <motion.div initial={{ opacity: 0, x: 40 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.9, delay: 0.15 }}>
            <div className="w-12 h-[2px] bg-emerald-500 mb-6" />
            <p className="text-emerald-500/60 text-xs uppercase tracking-[0.3em] mb-3" style={{ fontFamily: 'var(--font-sans)' }}>The Difference</p>
    ...
    <section ref={containerRef} className="bg-slate-950 py-24 sm:py-32 lg:py-40 overflow-hidden">
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12 mb-12 sm:mb-16">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.8 }} className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-6">
          <div>
            <div className="w-12 h-[2px] bg-emerald-500 mb-6" />
            <p className="text-emerald-500/60 text-xs uppercase tracking-[0.3em] mb-3" style={{ fontFamily: 'var(--font-sans)' }}>Selected Works</p>
          ...
          <Link to="/projects" className="group text-white/30 text-xs uppercase tracking-[0.2em] flex items-center gap-2 hover:text-emerald-500 transition-colors" style={{ fontFamily: 'var(--font-sans)' }}>
    ...
    <section ref={ref} className="bg-slate-900 py-24 sm:py-32 lg:py-40 overflow-hidden">
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.8 }} className="text-center mb-16 sm:mb-20">
          ...
          <p className="text-emerald-500/60 text-xs uppercase tracking-[0.3em] mb-3" style={{ fontFamily: 'var(--font-sans)' }}>How It Works</p>
          <h2 className="font-heading text-white leading-[0.95] italic" style={{ fontSize: 'clamp(2rem, 4.5vw, 3.5rem)' }}>
            Simple <span className="text-emerald-500">Process</span>
          </h2>
        </motion.div>

        <div className="relative grid md:grid-cols-4 gap-8 sm:gap-10">
          {/* Connecting line */}
          <div className="hidden md:block absolute top-12 left-[12.5%] right-[12.5%] h-[1px] bg-gradient-to-r from-transparent via-emerald-500/20 to-transparent z-0" />
          ...
          {steps.map((step, i) => (
            <motion.div key={step.num} initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.7, delay: i * 0.15 }} className="relative z-10 text-center">
              <div className="w-16 h-16 mx-auto mb-6 border border-emerald-500/30 flex items-center justify-center bg-slate-950 group-hover:bg-emerald-500/10 transition-colors">
                <span className="text-emerald-500 font-heading text-2xl italic">{step.num}</span>
              </div>
      ...
      <div className="relative z-10 max-w-4xl mx-auto px-5 sm:px-8 lg:px-12">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.8 }} className="text-center">
          ...
          <div className="flex items-center justify-center gap-6 mt-12">
            <button onClick={prev} className="w-10 h-10 border border-white/10 flex items-center justify-center text-white/30 hover:text-emerald-500 hover:border-emerald-500/30 transition-colors duration-300" aria-label="Previous">
              <CaretLeft size={16} />
            </button>
            <div className="flex gap-2">
              {homeTestimonials.map((_, i) => (
                <button key={i} onClick={() => setActive(i)} className={`h-[2px] transition-all duration-500 ${i === active ? 'w-10 bg-emerald-500' : 'w-3 bg-white/10 hover:bg-white/25'}`} aria-label={`Testimonial ${i + 1}`} />
              ))}
            </div>
            <button onClick={next} className="w-10 h-10 border border-white/10 flex items-center justify-center text-white/30 hover:text-emerald-500 hover:border-emerald-500/30 transition-colors duration-300" aria-label="Next">
              <CaretRight size={16} />
            </button>
          </div>
        </motion.div>
      ...
      <div className="relative z-20 max-w-5xl mx-auto px-5 sm:px-8 lg:px-12 text-center">
        <motion.div initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: '-80px' }} transition={{ duration: 1 }}>
          ...
          <h2 className="font-heading text-white leading-[0.92] italic mb-8" style={{ fontSize: 'clamp(2.5rem, 8vw, 6rem)' }}>
            CLEAN SPACES<br /><span className="text-emerald-500">HEALTHY LIVES</span>
          </h2>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <Link to="/contact" className="group relative inline-flex items-center gap-3 bg-emerald-500 text-white px-8 py-4 text-sm uppercase tracking-[0.15em] font-semibold transition-all duration-500 hover:bg-emerald-400 hover:shadow-xl hover:shadow-emerald-500/25" style={{ fontFamily: 'var(--font-sans)' }}>
              {homeCta.ctaPrimary}
              <ArrowRight size={16} className="transition-transform group-hover:translate-x-1" />
            </Link>
          ...
          <div className="mt-16 pt-8 border-t border-white/5 flex flex-col sm:flex-row items-center justify-center gap-6 text-xs text-white/25 uppercase tracking-[0.15em]" style={{ fontFamily: 'var(--font-sans)' }}>
            <a href={`tel:${business.phoneRaw}`} className="flex items-center gap-2 hover:text-emerald-500 transition-colors">
              <Phone size={14} /> {business.phone}
            </a>
            <span className="hidden sm:inline text-white/10">|</span>
            <a href={`mailto:${business.email}`} className="hover:text-emerald-500 transition-colors">{business.email}</a>
            <span className="hidden sm:inline text-white/10">|</span>
